...
cellFromPointOrPolygon = function(r, p, type){
	switch(type,
		'1' = {
			...
			print(paste(p$ID, as.character(extent(centr)[2:3]), collapse = ""))
			centr_cell = cellFromXY(r, centr)
			print(paste(p$ID, as.character(extent(centr)[2:3]), centr_cell, collapse = ""))
			pol_cells = cellFromPolygon(r, p, weights=F)[[1]]
			print(paste(p$ID, as.character(extent(centr)[2:3]), centr_cell, length(pol_cells), collapse = ""))
